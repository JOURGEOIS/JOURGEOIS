<template>
  <div class="the-community-form-view">
    ...
    <section>
      <!-- 폼 풀러오기 -->
      <the-community-form
        id="the-community-form"
      ></the-community-form>
      <!-- 이용규칙 -->
      <div class="the-community-notice">
        <the-community-notice-button></the-community-notice-button>
        <p>주류주아는 누구나 기분 좋게 참여할 수 있는 커뮤니티를 만들기 위해 커뮤니티 이용규칙을 제정하여 운영하고 있습니다.</p>
...
const errorStatus = computed(
  () => store.getters["createFeed/getAlertStatus"]
);
...
// 모달 초기화 및 db 요청
onBeforeMount(() => {
  store.dispatch("createFeed/getCommunityDetail", route.params.feedId);
  store.dispatch("feedDescInfo/changeAlertStatus", false);
});
